...
(defn books-in-range
  "Returns a list with vectors containing the title and author
  of the books published within the given time range."
  [s start end]
  (map #(vector (:title %) (:author %))
    (sort-by :title
      (filter (fn [x] (<= start (:year x) end))
        s))))

  ; First Draft:
  ; (sort (fn [a b] (< (:year a) (:year b)))
  ;   (filter (fn [x] (and (>= (:year x) start) (<= (:year x) end)))
  ;     s)))
